...
def main(args):
    ...
    pipe = sklearn.pipeline.Pipeline([('minmax', sklearn.preprocessing.MinMaxScaler()), ('poly', sklearn.preprocessing.PolynomialFeatures(
    ...
    params = [{'poly__degree': [1, 2], 'logreg__C':[
        0.01, 1, 100], 'logreg__solver':['lbfgs', 'sag']}]
    ...
    clf = sklearn.model_selection.GridSearchCV(pipe, params)
    clf.fit(X_train, Y_train)
    predictions = clf.predict(X_train)

    test_accuracy = sum(clf.predict(X_test) == Y_test) / Y_test.shape[0]
